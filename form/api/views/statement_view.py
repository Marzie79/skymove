    ...
    def list(self, request, *args, **kwargs):
        ...
        if consignee.exists():
            statements = Statement.objects.filter(
                shipment__consignee__name=consignee_name,
                consignee__name=consignee_name,
                currency=currency,
            )
            shipments = (
                Shipment.objects.filter(
                    base_form__consignee=consignee.first(), base_form__currency=currency
                )
                .exclude(pk__in=statements.values_list("shipment__id", flat=True))
            ...
            courier = (
                Courier.objects.filter(
                    base_form_courier__consignee=consignee.first(), currency=currency
                )
                .exclude(
                    pk__in=statements_courier.values_list("courier__id", flat=True)
                )
                .order_by("-base_form_courier__date")
            )
            ...
            for item in range(0, max):
                if shipments or courier:
                    if (
                        shipments
                        and courier
                        and shipments.first().base_form.date
                        > courier.base_form_courier.date
                    ) or (shipments and not courier):
                        obj = Statement()
                        ...
                        obj.balance = (last.balance + shipments.first().total_sell) - (
                            last.charge if last.charge else 0
                        )
                        ...
                        last = Statement.objects.all().order_by("-order").first()
                        obj.balance = (last.balance + courier.first().total_buy) - (
                            last.charge if last.charge else 0
                        )
                        obj.order = (
                            Statement.objects.all().order_by("-order").first() + 1
                        )
                        courier = courier.exclude(courier.first())
                    obj.save()

        need_obj = Statement.objects.filter(
            consignee__name=consignee_name, currency=currency
        )
        ...
        for item in need_obj:
            ...
            if not consignee_order_no:
                if item.shipment:
                    ...
                    consignee_order_no = (
                        item.courier.base_form_courier.consignee_order_no
                    )
            credit = ""
            if not credit:
                if item.shipment:
